...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <p class="text-xs font-semibold text-[#1B2A5E] uppercase tracking-widest mb-1">USER DASHBOARD</p>
            <h1 class="text-3xl font-bold text-gray-900">Your Reading Activity</h1>
        </div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-8">
        <div class="bg-white border border-gray-200 rounded-2xl p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400">/ 05 Limit</p>
            </div>
        </div>
        <div class="bg-white border {{ $overdueBooks > 0 ? 'border-red-300' : 'border-gray-200' }} rounded-2xl p-5 flex items-center gap-4">
            ...
            <div>
                <p class="text-xs text-gray-500">Overdue Books</p>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-6 mb-6">
        <h2 class="font-bold text-gray-900 text-lg mb-4 flex items-center gap-2">📋 Active Loans</h2>
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center gap-4 p-4 rounded-xl border {{ $loan->status === 'overdue' ? 'border-red-200 bg-red-50' : 'border-gray-100' }}">
                ...
                <div class="text-center hidden md:block">
                    <p class="text-xs text-gray-400 uppercase tracking-wide">Borrowed Date</p>
                    <p class="text-sm font-medium">{{ $loan->borrowed_date->format('M d, Y') }}</p>
                </div>

                <div class="text-center hidden md:block">
                    <p class="text-xs text-gray-400 uppercase tracking-wide">Return Deadline</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-6">
            <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2">🕐 Recent History</h2>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-xs text-gray-400 uppercase tracking-wide border-b border-gray-100">
                        <th class="text-left pb-2 font-medium">Book Title</th>
                        <th class="text-left pb-2 font-medium">Returned</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr>
                        ...
                        <td class="py-3 text-gray-500 whitespace-nowrap">{{ $loan->returned_date?->format('M d, Y') }}</td>
                        <td class="py-3">
                            @php
        ...
        {{-- SAVED FOR LATER --}}
        <div class="bg-white border border-gray-200 rounded-2xl p-6">
            <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2">★ Saved for Later</h2>
                ...
                <div class="flex gap-3 items-start">
                    ...
                    <div class="flex-1">
                        ...
                        <span class="{{ $wish->book->available_copies > 0 ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700' }} text-[10px] font-bold px-2 py-0.5 rounded-full mt-1 inline-block">
                            {{ $wish->book->available_copies > 0 ? 'AVAILABLE' : 'WAITLIST' }}
                        </span>
                    </div>
                </div>
                @endforeach
